...
interface InsideTrackPieceFormProps {
	memberPostId?: string;
}

export function InsideTrackPieceForm({ memberPostId }: InsideTrackPieceFormProps) {
	...
	const handleSaveDraft = form.handleSubmit(async (values) => {
		try {
			...
			if (!isEdit) router.replace(getAdminPath(`/inside-track/${id}`));
		} catch {
		...
		<div className="gap-4 grid grid-cols-1">
			<div className="mb-2 flex justify-start">
				<Button variant="link" size="sm" asChild className="px-0">
					<Link href={getAdminPath("/inside-track")}>
			...
			<Card>
				<CardHeader className="gap-2 flex flex-row items-center justify-between">
					<CardTitle>
						{isEdit
							? t("admin.insideTrack.editTitle")
							: t("admin.insideTrack.createTitle")}
					</CardTitle>
					{isPublished && (
				...
				<CardContent>
					<Form {...form}>
						<form className="gap-6 grid grid-cols-1">
							{/* Persistent audience banner — Inside Track is educational content, all members */}
							<div className="gap-3 border-blue-200 bg-blue-50 p-3 text-blue-900 dark:border-blue-900/50 dark:bg-blue-950/40 dark:text-blue-100 flex items-center rounded-md border">
								...
								<div className="text-sm">{t("admin.insideTrack.audience")}</div>
							</div>
							...
							{!isPublished && (
								<label className="gap-2 text-sm flex items-center">
									...
									/>
									{t("admin.insideTrack.notifyMembers")}
									<span className="text-xs text-muted-foreground">
										{t("admin.insideTrack.notifyMembersHint")}
									</span>
								</label>
							)}
