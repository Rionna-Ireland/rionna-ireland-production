...
describe("getMemberPost", () => {
	...
	it("returns a normalized post from /spaces/{spaceId}/posts/{postId}", async () => {
		...
		const res = await call(getMemberPost, { organizationId: ORG_ID, spaceId: "2713068", postId: "34130292" }, ctx);
	...
	it("returns null on a 404 (missing/unauthorized post)", async () => {
		...
		const res = await call(getMemberPost, { organizationId: ORG_ID, spaceId: "1", postId: "2" }, ctx);
		expect(res).toBeNull();
	});
});
