...
/**
 * Org-wide POLL push on admin publish. Atomic claim on Poll.notifiedAt (news
 * pattern) so concurrent publishes can't double-send; the claim is released on
 * total delivery failure or a throw so a re-publish can retry. Never throws.
 */
export async function notifyPollPublished(input: NotifyPollPublishedInput): Promise<void> {
	...
	try {
		...
		logger.error("[Polls] publish notify threw", { pollId: input.pollId, error });
		await releasePollNotification(input.pollId);
	}
}
